...
def test_index_performance():
    ...
    with app.app_context():
        try:
            ...
            start_time = time.time()
            chats = (
                Chat.query.filter_by(user_id=1)
            ...
            start_time = time.time()
            messages = (
                Message.query.filter_by(chat_id=1).order_by(Message.timestamp).all()
            )
        ...
        try:
            ...
            with db.engine.connect() as conn:
                ...
                if db_type == "postgresql":
                    ...
                    logger.warning(
                        f"Query plan analysis not supported for database type: {db_type}"
                    )
    ...
    with app.app_context():
        try:
            ...
            if not test_user:
                ...
                # Create test user
                test_user = User(
                    username="testuser", email="test@example.com", is_approved=True
                )

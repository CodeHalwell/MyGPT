...
from chat_handler import MODEL_MAPPING, generate_chat_summary, suggest_tags
...
from models import Chat, Message, Tag, User
...
def index():
    ...
    chat_stats = {
        ...
        "avg_messages_per_chat": (
            round(total_messages / total_chats, 1) if total_chats > 0 else 0
        ),
    }
...
def register():
    ...
    if request.method == "POST":
        # Validate form data using Marshmallow schema
        validated_data, errors = validate_form_data(
            UserRegistrationSchema, request.form
        )
        ...
        if (
            ...
            and user.email == app.config["ADMIN_EMAIL"]
        ):
            user.is_admin = True
        ...
        if not user.is_admin:
            ...
            if not email_sent:
                flash(
                    "Registration successful but email notification could not be sent."
                )
            else:
                flash(
                    "Your registration is pending approval from an administrator. You will receive an email when your account is approved."
                )

            admin_notified = send_admin_notification_email(user.email, user.username)
            if not admin_notified:
                app.logger.error(
                    f"Failed to send admin notification for new user registration: {user.username}"
                )
...
def get_messages(chat_id):
    ...
    messages = [
        {"content": msg.content, "role": msg.role, "model": msg.model}
...
def save_message(chat_id):
    ...
    try:
        ...
        message = Message(chat_id=chat_id, content=message_content, role="user")
...
def stream_response(chat_id):  # Temporarily removed @login_required
    ...
    def generate():
        ...
        message = Message(
            chat_id=chat_id, content=complete_response, role="assistant", model=model
        )
        db.session.add(message)

        if len(messages) <= 1:
            chat.title = generate_chat_summary(
...
def toggle_admin(user_id):
    ...
    validated_data, errors = validate_json_data(
        AdminToggleSchema, request.get_json() or {}
    )
...
def create_tag():
    ...
    # Validate JSON data using Marshmallow schema
    validated_data, errors = validate_json_data(
        CreateTagSchema, request.get_json() or {}
    )
...
def update_tag(tag_id):
    ...
    # Validate JSON data using Marshmallow schema
    validated_data, errors = validate_json_data(
        UpdateTagSchema, request.get_json() or {}
    )
...
def update_password():
    ...
    # Validate form data using Marshmallow schema
    validated_data, errors = validate_form_data(
        UpdatePasswordSchema, form_data, context
    )

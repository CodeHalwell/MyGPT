    ...
    with db.engine.connect() as conn:
        ...
        try:
            if db_type == "postgresql":
                ...
                conn.execute(
                    text(
                        "ALTER TABLE message ADD COLUMN IF NOT EXISTS model VARCHAR(50)"
                    )
                )
                conn.execute(
                    text(
                ...
                if "model" not in columns:
                    conn.execute(
                        text("ALTER TABLE message ADD COLUMN model VARCHAR(50)")
                    )
                ...
                if "reset_token" not in columns:
                    conn.execute(
                        text('ALTER TABLE "user" ADD COLUMN reset_token VARCHAR(100)')
                    )
...
def run_index_migrations():
    ...
    with db.engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(
                text('CREATE INDEX IF NOT EXISTS idx_user_email ON "user" (email)')
            )
            conn.execute(
                text(
            ...
            conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_chat_user_id ON chat (user_id)")
            )
            conn.execute(
                text(

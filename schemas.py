...
class UserRegistrationSchema(Schema):
    ...
    username = fields.Str(
        ...
        validate=[
            validate.Length(
                min=3, max=64, error="Username must be between 3 and 64 characters"
            ),
            validate.Regexp(
                r"^[a-zA-Z0-9_-]+$",
    ...
    email = fields.Email(
        ...
        validate=validate.Length(
            max=120, error="Email must be less than 120 characters"
        ),
    )
    password = fields.Str(
        required=True,
...
class UpdatePasswordSchema(Schema):
    ...
    confirm_password = fields.Str(
        required=True,
        validate=validate.Length(
            min=1, max=128, error="Password confirmation is required"
        ),
    )
...
class CreateTagSchema(Schema):
    ...
    name = fields.Str(
        ...
        validate=[
            validate.Length(
                min=1, max=50, error="Tag name must be between 1 and 50 characters"
            ),
            validate.Regexp(
                r"^[a-zA-Z0-9\s\-_]+$",

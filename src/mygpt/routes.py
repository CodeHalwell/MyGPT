...
from .services.multi_provider_chat_handler import get_ai_response_stream
from .services.chat_handler import generate_chat_summary, suggest_tags, MODEL_MAPPING
from .services.email_handler import (
...
def init_routes(app):
    ...
    def reject_user(user_id):
        ...
        if send_approval_email(user.email, user.username, approved=False):
            return jsonify({'status': 'success'})
        else:
    ...
    def update_tag(tag_id):
        ...
        tag = Tag.query.get_or_404(tag_id)
        tag.name = validated_data['name'].strip().lower()
    ...
    def update_password():
        # Validate form data using Marshmallow schema
        validated_data, errors = validate_form_data(UpdatePasswordSchema, request.form)
